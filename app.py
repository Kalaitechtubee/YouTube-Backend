...
def get_common_quality(streams):
    quality_map = {}
    quality_labels = {
        "2160p": "4K",
        "1440p": "2K",
    ...
    for stream in streams:
        ...
        if not res or res == '0p':
            continue

        # Add label
        stream['label'] = quality_labels.get(res, "")

        if res not in quality_map:
            quality_map[res] = stream
        else:
            # Logic: Prefer MP4, then prefer streams that already have audio
            existing = quality_map[res]
            
            # If current is mp4 and existing isn't, prefer this one
            if stream["extension"] == "mp4" and existing["extension"] != "mp4":
                quality_map[res] = stream
            # If extensions are same, prefer the one with audio
            elif stream["extension"] == existing["extension"]:
                if stream.get('has_audio') and not existing.get('has_audio'):
                    quality_map[res] = stream

    # convert to list & sort
    final_streams = list(quality_map.values())

    def sort_key(s):
        try:
            res_str = str(s.get('resolution', '0p')).lower()
            # extract only digits
            digits = ''.join(filter(str.isdigit, res_str))
            return int(digits) if digits else 0
        except (ValueError, AttributeError):
            return 0

    final_streams.sort(key=sort_key, reverse=True)
...
def get_common_audio_quality(streams):
    quality_map = {}
    audio_labels = {
        "160kbps": "High Quality",
        "128kbps": "Medium Quality",
        "70kbps": "Good Quality",
        "50kbps": "Low Quality",
        "48kbps": "Very Low"
    }
    ...
    for stream in streams:
        abr = stream.get("quality")
        if not abr or abr == '0kbps':
            continue

        # Add label
        stream['label'] = audio_labels.get(abr, "Standard Quality")

        if abr not in quality_map:
            quality_map[abr] = stream
        else:
            # Logic: Prefer M4A over WEBM for audio
            existing = quality_map[abr]
            if stream["extension"] == "m4a" and existing["extension"] != "m4a":
                quality_map[abr] = stream
    ...
    def sort_key(s):
        try:
            return int(s['quality'].replace('kbps', ''))
